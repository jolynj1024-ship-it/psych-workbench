...
function ghSetSha(s) {
  if (s) localStorage.setItem(GH_PREFIX + 'sha', s);
}
function ghSetLast() {
  localStorage.setItem(GH_PREFIX + 'last', new Date().toISOString());
}
function ghApiUrl(conf) {
  return 'https://api.github.com/repos/' + conf.repo + '/contents/' + conf.path;
}
...
function ghNormalize(d) {
  ...
  COLS.forEach(c => {
    base[c] = Array.isArray(d[c]) ? d[c] : [];
  });
  if (d.archive && typeof d.archive === 'object') {
    base.archive = {
      records: Array.isArray(d.archive.records) ? d.archive.records : [],
      todos: Array.isArray(d.archive.todos) ? d.archive.todos : []
    };
  }
  return base;
}
function ghMirror(data) {
  try {
    ...
    COLS.forEach(c => {
      if (Array.isArray(data[c])) lsSet(c, data[c]);
    });
...
function byTs(x) {
  return x && (x.updatedAt || x.createdAt || '');
}
function unionById(a, b) {
  ...
  a.concat(b).forEach(x => {
    if (x && x.id) {
      ...
      if (!p || byTs(x) > byTs(p)) map.set(x.id, x);
    }
  });
...
function ghMerge(local, remote) {
  ...
  COLS.forEach(c => {
    out[c] = unionById(local[c] || [], remote[c] || []);
  });
  out.archive = {
...
let ghCache = null,
  ghCacheSha = null,
  ghLoading = null,
  ghPushChain = Promise.resolve();
async function ghFetch() {
  const conf = ghConf();
  if (!conf.token || !conf.repo) throw new Error('GitHub 未配置');
  const res = await fetch(ghApiUrl(conf) + '?ref=' + conf.branch, {
    headers: ghHeaders(conf),
    signal: AbortSignal.timeout(8000)
  });
  if (res.status === 404) {
    ghCache = ghNormalize(null);
    ghCacheSha = '';
    ghSetLast();
    return ghCache;
  }
...
function ghEnsure() {
  if (!ghLoading) ghLoading = ghFetch().catch(e => {
    ghLoading = null;
    throw e;
  });
  return ghLoading;
...
async function ghPush() {
  ...
  const put = async sha => {
    const body = {
      message: '🧠 工作台数据更新 ' + new Date().toISOString(),
      content,
      branch: conf.branch
    };
    if (sha) body.sha = sha;
    const r = await fetch(ghApiUrl(conf), {
      method: 'PUT',
      headers: ghHeaders(conf, {
        'Content-Type': 'application/json'
      }),
    ...
    if (r.ok) {
      ...
      ghCacheSha = j.content.sha;
      ghSetSha(j.content.sha);
      ghSetLast();
      return true;
    }
    return r;
  ...
  ghPushChain = ghPushChain.then(async () => {
    for (let i = 0; i < 4; i++) {
      ...
      if (r && r.status === 409) {
        // 冲突：合并远端后再提交
        const j = await (await fetch(ghApiUrl(conf) + '?ref=' + conf.branch, {
          headers: ghHeaders(conf),
          signal: AbortSignal.timeout(8000)
        })).json();
...
async function ghApi(path, method, body) {
  ...
  if (col === 'worktypes') {
    ...
    if (method === 'PUT') {
      ghCache.workTypes = body || DEFAULT_WORKTYPES;
      await ghPush();
      return ghCache.workTypes;
    }
  }
  if (col === 'archive') {
    if (method === 'GET') return ghCache.archive || {
      records: [],
      todos: []
    };
    if (method === 'POST') {
      ...
      const moveT = todos.filter(t => t.done),
        keepT = todos.filter(t => !t.done);
      ghCache.archive = ghCache.archive || {
        records: [],
        todos: []
      };
      ghCache.archive.records = moveR.concat(ghCache.archive.records || []);
      ghCache.archive.todos = moveT.concat(ghCache.archive.todos || []);
      ghCache.records = keepR;
      ghCache.todos = keepT;
      await ghPush();
      return {
        movedRecords: moveR.length,
        movedTodos: moveT.length
      };
    }
  }
  if (COLS.includes(col)) {
    ...
    if (method === 'POST' && !id) {
      ...
      ghCache[col] = ghCache[col] || [];
      ghCache[col].unshift(item);
      await ghPush();
      return item;
    }
    if (method === 'PUT' && id) {
      const arr = ghCache[col] || [];
      const idx = arr.findIndex(x => x.id === id);
      if (idx < 0) throw new Error('未找到记录');
      delete body.id;
      delete body.createdAt;
      arr[idx] = Object.assign({}, arr[idx], body, {
...
function startGhPoller() {
  if (window.__ghPoller) return;
  window.__ghPoller = true;
  setInterval(async () => {
    const conf = ghConf();
    ...
    try {
      ...
      if (j.sha !== ghCacheSha) {
        ...
        ghMirror(ghCache);
        ghSetLast();
        window.dispatchEvent(new CustomEvent('pw-remote-update', {
          detail: {
            sha: j.sha
          }
        }));
...
function localApi(path, method, body) {
  ...
  if (col === 'archive') {
    ...
    if (method === 'POST') {
      ...
      arch.records = moveR.concat(arch.records || []);
      arch.todos = moveT.concat(arch.todos || []);
...
window.API = async function (path, method = 'GET', body) {
  ...
  if (conf.enabled && conf.token && conf.repo) {
    try {
      return await ghApi(path, method, body);
    } catch (e) {
...
window.GHSync = {
  async test() {
    const c = ghConf();
    if (!c.token || !c.repo) throw new Error('请先填写仓库与令牌');
    ghLoading = null;
    ghCache = null;
    ghCacheSha = null;
    await ghEnsure();
  ...
  async uploadLocal() {
    const c = ghConf();
    if (!c.token || !c.repo) throw new Error('请先配置 GitHub');
    ghCache = ghNormalize(localSnapshot());
    ghCacheSha = c.sha || '';
    await ghPush();
    return true;
  },
  ...
  lastSync() {
    return localStorage.getItem(GH_PREFIX + 'last') || '';
  }
};
