...
window.API = async function (path, method = 'GET', body) {
  ...
  if (online) {
    try {
      ...
      // 服务端临时故障，降级到 localStorage
      if (!e.message.includes('fetch') && !e.message.includes('network') && !e.message.includes('timeout')) throw e;
      serverOk = false;
    }
  }
  // ---- localStorage 降级模式 ----
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = e => {
      try {
        ...
        if (!data.version) throw new Error('无效的数据文件');
